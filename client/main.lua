...
AddEventHandler(
    ...
    function(item, count, showNotification)
...
AddEventHandler(
    ...
    function(weaponName, weaponComponent)
        local playerPed = PlayerPedId()
        local weaponHash = GetHashKey(weaponName)
...
AddEventHandler(
    "esx:setWeaponAmmo",
    function(weaponName, weaponAmmo)
        local playerPed = PlayerPedId()
        local weaponHash = GetHashKey(weaponName)
...
AddEventHandler(
    "esx:setWeaponTint",
    function(weaponName, weaponTintIndex)
        local playerPed = PlayerPedId()
        local weaponHash = GetHashKey(weaponName)
...
AddEventHandler(
    "esx:createPickup",
    function(pickupId, label, playerId, type, name, components, tintIndex)
        ...
        while true do
            ...
            for k, v in ipairs(Config.Weapons) do
                ...
                if HasPedGotWeapon(playerPed, weaponHash, false) then
                    local ammo, tintIndex, weaponComponents =
                        GetAmmoInPedWeapon(playerPed, weaponHash),
                        GetPedWeaponTintIndex(playerPed, weaponHash),
                        {}
                ...
                if distance < 5 then
                    ...
                    if distance < 1 then
                        if IsControlJustReleased(0, 38) then
                            if
                                IsPedOnFoot(playerPed) and (closestDistance == -1 or closestDistance > 3) and
                                    not v.inRange
                             then
                                v.inRange = true
