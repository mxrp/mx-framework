...
function onPlayerJoined(playerId)
    ...
    if identifier then
        -- return characters beginning with your ID
        MySQL.Async.fetchScalar(
            ...
            function(result)
                ...
                TriggerEvent("loadCharacters", playerId, characters)
...
function loadESXPlayer(identifier, playerId)
    -- identifier is now appended with `::charId`
    local tasks = {}
    ...
    table.insert(
        ...
        function(cb)
            MySQL.Async.fetchAll(
                "SELECT name, money FROM user_accounts WHERE identifier = @identifier",
                {
                    ["@identifier"] = identifier
                },
                function(accounts)
    ...
    table.insert(
        ...
        function(cb)
            MySQL.Async.fetchAll(
                "SELECT job, job_grade, `group`, loadout, position, inventory FROM users WHERE identifier = @identifier",
                {
                    ["@identifier"] = identifier
                },
                function(result)
                    ...
                    if ESX.DoesJobExist(job, grade) then
                        ...
                        print(
                            ("[es_extended] [^3WARNING^7] Ignoring invalid job for %s [job: %s, grade: %s]"):format(
                                identifier,
                                job,
                                grade
                            )
                    ...
                    if result[1].inventory and result[1].inventory ~= "" then
                        ...
                        for name, count in pairs(inventory) do
                            ...
                            if item then
                                ...
                                print(
                                    ('[es_extended] [^3WARNING^7] Ignoring invalid item "%s" for "%s"'):format(
                                        name,
                                        identifier
                                    )
                                )
                    ...
                    if result[1].position and result[1].position ~= "" then
                        ...
                        print(
                            '[es_extended] [^3WARNING^7] Column "position" in "users" table is missing required default value. Using backup coords, fix your database.'
                        )
